...
- Emby 音乐播放已显式补官方 `/Audio/{Id}/universal`，普通请求复用现有流式读取、STRM allowlist 和用户可见权限边界，`TranscodingProtocol=hls` 请求会创建 audio-only HLS 转码 session 并跳转到 `/emby/Audio/{Id}/master.m3u8`；Universal Audio 查询参数会先做 allowlist/范围规范化，码率兼容 `MaxStreamingBitrate` 和 `AudioBitRate`（前者优先，后者作为 fallback），完整渐进式音频转码语义后续继续补齐。
