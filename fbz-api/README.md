...
- Emby `Artists`、`Artists/AlbumArtists` 和 `Artists/{Name}` 按当前用户可见音乐内容聚合艺术家，支持分页、搜索和基础名称排序
...
| `METADATA_PROVIDERS` | `tmdb,tvdb,fanart` | 元数据 provider 顺序；worker 会记录每个 provider 的 matched / not_matched / skipped 边界，当前仅 TMDB 执行远端查询 |
...
| `TVDB_API_KEY` | 空 | TVDB API key，provider 边界已保留 |
| `TVDB_API_BASE_URL` | `https://api4.thetvdb.com/v4` | TVDB API 地址，可替换为镜像地址 |
| `FANART_API_KEY` | 空 | Fanart API key，provider 边界已保留 |
...
- `GET /api/admin/users`：列出用户、角色、全局下载/转码/新增设备登录策略、设备数量和活跃 session 数。
...
- `GET /api/admin/users/{userId}/libraries`：列出用户对所有媒体库的 `canView`、`canDownload`、`canTranscode` 配置值和叠加全局策略后的 effective 权限。
- `PUT /api/admin/users/{userId}/libraries/{libraryId}/permissions`：替换用户在单个媒体库上的权限配置，使用唯一约束 upsert。
- `GET /api/admin/jobs`：按最新任务列出通用 job 队列状态、运行租约、attempts、payload 和最后错误。
...
- `GET /api/admin/scheduled-tasks`：列出 core/plugin 计划任务、下次运行时间、活跃运行数、最近运行记录、失败次数和最后错误。
- `GET /api/admin/scheduled-tasks/{taskKey}/runs`：查看单个计划任务最近运行历史、租约状态、耗时和错误信息。
- `POST /api/admin/scheduled-tasks/{taskKey}/run`：手动触发一个已启用的计划任务，复用 scheduler 的任务执行逻辑。
- `GET /api/admin/transcoding-sessions`：列出最近转码会话、运行租约、attempts 和错误信息。
- `POST /api/admin/transcoding-sessions/{sessionId}/cancel`：取消 queued / running 转码会话。
- `GET /api/admin/notification-targets`：列出通知目标，敏感配置脱敏。
...
- `GET /api/admin/notification-requests`：列出插件通知请求和最终状态。
- `GET /api/admin/notification-requests/{requestId}/attempts`：查看单个通知请求的目标投递尝试。
- `POST /api/admin/notification-requests/{requestId}/retry`：把 `failed` / `discarded` 通知请求重新入队，已成功投递过的目标会被 worker 跳过。
- `GET /api/admin/plugins/capabilities`：读取当前插件宿主能力清单，用于后台安装前校验 manifest runtime、权限、hook 事件、HTTP scheme 和 Host API 契约。
...
- `GET /api/admin/plugin-dispatches`：列出插件 hook 派发 outbox 事件和失败原因。
- `GET /api/admin/plugin-dispatches/{dispatchId}/runs`：查看单个插件派发事件的执行记录。
- `GET /api/admin/plugin-host-api-calls`：列出插件 Host API 调用审计，支持 `pluginId`、`executionRunId`、`statusCode` 和 `limit` 查询参数。
- `GET /api/admin/plugin-execution-runs/{runId}/host-api-calls`：查看单次插件执行中的 Host API 调用明细。
...
- `GET /api/plugin/capabilities`：读取当前 host 支持的插件 API 版本、runtime、HTTP scheme、权限、hook 事件和 Host API 能力清单。
...
一等 HTTP 通知插件示例位于 `examples/plugins/http-notification-bridge`，展示 manifest 权限声明、HTTP 签名校验、幂等去重和受控通知 Host API 调用。
