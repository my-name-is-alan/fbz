...
- Emby `POST Collections`、`POST/DELETE Collections/{Id}/Items` 和 `Collections/{Id}/Items/Delete` 已补认证保护的 CollectionService 写入口探测，受控解析官方 `Name`、`IsLocked`、`ParentId` 和逗号分隔 `Ids`，query 字段兼容 PascalCase、lower-camel 和 snake_case；当前返回冲突，避免真实合集模型、权限审计和成员写入未设计完成前制造未落库成功状态
...
- Emby `Persons` 和 `Persons/{Name}` 按当前用户可见媒体项聚合人物，支持 lower-bound 分页、搜索、人物类型过滤和基础名称排序，避免对大库人物聚合做全量 `count(*) over()`，常见 query 字段兼容 PascalCase、lower-camel 和 snake_case，并钳制病态超大 `StartIndex`
