...
- 后台每条子路由对应 `src/views/admin/**/index.vue` 独立页面（如 `/admin/users` → `views/admin/users/index.vue`，`/admin` 控制面板 → `views/admin/index.vue`）；页面用共享的 `AdminPageShell`（标题横幅 + 内容插槽）包裹 `components/admin/` 下的域组件，新增后台页先建 view 再在路由表登记，不要再引入集中式切换器视图。
...
- 设计基调：家庭媒体中心（类 Emby）分层深色——近黑带微蓝调背景（`--fbz-color-bg: #0e0f13`，避免死黑）+ 面板逐级提亮 + 单一主题色 `--fbz-color-brand-500: #1ed760`。主题绿只用于强调态（导航激活、主按钮、进度条、悬停高亮），其余一律白/灰阶；禁止多彩混用、装饰性渐变（媒体图上的黑色可读性渐晕不算装饰渐变）。圆角统一走 token：卡片/海报 `--fbz-radius-card: 10px`、控件 `--fbz-radius-control: 8px`，不要硬写像素圆角。媒体卡片是「悬浮海报」风格：无边框、海报承担圆角与 `--fbz-shadow-card` 阴影，悬停上浮加深阴影并浮出品牌色播放钮；清晰度徽章为黑色半透明胶囊角标（不做彩色标签）。Orbitron 展示体只用于品牌 Logo 与统计数字，媒体标题/正文一律 `--fbz-font-sans`。
